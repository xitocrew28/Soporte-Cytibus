...
void cobrarEV2() {
  ...
  /*
   * TODO: agregar nuevas validaciones o datos en la tarjeta por ejemplo
   * si habra transbordos y esas cosas
   */
  ...
  byte llaveTipo[18];
  byte sizellaveTipo = 18;

  byte appNom[3]           = {0X01, 0X00, 0X00};
  byte fileOffsetLenEsIdNom[7] = {0X01, 0X18, 0X00, 0X00, 0X20, 0X00, 0X00};
  byte etidNom[32];
  byte sizeetidNom = 32;  
  ...
  mfrc522.MIFARE_DESFIRE_ReadDataInnoParametrosPago(&tag, app, fileOffsetLen, llaveTipo, sizellaveTipo);
  //delay(50);

  mfrc522.MIFARE_DESFIRE_ReadDataInnoParametrosPago(&tag, appNom, fileOffsetLenEsIdNom, etidNom, sizeetidNom);
  //delay(50);

  mfrc522.MIFARE_DESFIRE_ReadDataInnoParametrosPago(&tag, appNom, fileOffsetLenFecha, fecha, sizeFecha);
  //delay(50);
  ...
  //Serial.println("");
  for (byte i = 0; i < 18; i++) {
    Serial.print(llaveTipo[i], HEX);
  }

  for (byte j = 0; j < 32; j++) {
    Serial.print(etidNom[j], HEX);
  }

  for (byte j = 0; j < 6; j++) {
    Serial.print(fecha[j], HEX);
  }
  //Serial.println("");
  Serial.print("*");
  Serial.print(folioI);
